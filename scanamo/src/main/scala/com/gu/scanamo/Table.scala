...
case class Table[V: DynamoFormat](name: String) {
  ...
  /**
    ...
    * }}}
    *
    * To concatenate a list to the front or end of an existing list, use appendList/prependList:
    *
    * {{{
    ...
    * ...     _ <- fruits.update('kind -> "watermelon", appendList('sources -> List("China", "Turkey")))
    * ...     _ <- fruits.update('kind -> "watermelon", prependList('sources -> List("Brazil")))
